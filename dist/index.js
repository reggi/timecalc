...
function formatDateTime({ runtime, timezone, format, input }) {
  let parsedDateTime = DateTime2.fromFormat(input, format).setZone(timezone);
...
var TimeCalc = class {
  ...
  unixToReadable(results) {
    const date = DateTime3.fromMillis(results).setZone(this.timezone);
    return date.toLocaleString(DateTime3.DATETIME_MED);
  }
  resolveIn(value) {
